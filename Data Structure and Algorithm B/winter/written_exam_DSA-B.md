...
Updated 2302 GMT+8 Jun 13, 2025
...
7. 假设线性表中每个元素有两个数据项 key1 和 key2，现对线性表按以下规则进行排序：先根据数据项 key1 的值进行非递减排序；在 key1 值相同的情况下，再根据数据项 key2 的值进行非递减排序。
   ...
   > 这个问题考察的是**排序的稳定性**和**多关键字排序**的正确策略。
   >
   > ------
   >
   > **多关键字排序原则**
   >
   > 对于双关键字排序（key1 是主关键字，key2 是次关键字）：
   >
   > - **先按次关键字排序**
   > - **再按主关键字进行**稳定排序
   >
   > 为什么要先排序次关键字再稳定排序主关键字？因为主关键字排序会保持原来 key1 相等时 key2 的相对顺序（若使用稳定排序），从而满足题设的“key1 非递减，key1 相同则 key2 非递减”的要求。
   >
   > ------
   >
   > **回顾稳定性**
   ...
   >   → 先排序主关键字，错！而且直接选择排序不稳定，可能破坏之前排序结果 ❌
   > - **B. 先按 key2 值进行冒泡排序，再按 key1 值进行直接选择排序**
   >   → 虽然排序顺序对了（先次后主），但 key1 用的是 **不稳定** 的选择排序，会破坏 key2 的排序结果 ❌
   > - **C. 先按 key1 值进行直接选择排序，再按 key2 值进行冒泡排序**
   >   → 排序顺序错了，应先 key2 后 key1 ❌
   > - **D. 先按 key2 值进行直接选择排序，再按 key1 值进行冒泡排序**
   >   → 排序顺序对了（次关键字 → 主关键字），而且 key1 用的是**稳定排序冒泡** ✅
   >   → 虽然 key2 用的是不稳定的选择排序，但只要主关键字排序是稳定的，这仍是**正确策略**
   >
   > ------
   >
   > ✅ 正确答案：**D**
   ...
   > ------
   >
   > 如果能换成“先按 key2 值用**稳定排序**，再按 key1 值用**稳定排序**”，那是最好的。但在这四个选项里，**D 是唯一满足正确排序顺序 + 主关键字用稳定排序**的。
...
5. 已知二叉树的前序遍历结果 （先根周游序列） 为 ADC， 这棵二叉树的树型有 _ _ _ _ 种可能。**（5）**

   > 给定 n 个节点，所有不同的二叉树结构（只考虑形状，不考虑节点的值）**的数量，正好是第 **n 个卡特兰数（Catalan number）。
   >
   > ### 背后含义：
   >
   > 卡特兰数常用于计数一些“不考虑值只考虑结构”的组合结构，比如：
   >
   > - 有 n 个节点的不同**二叉树结构**
   > - 有 n 对括号的**合法配对方式**
   > - **栈操作**合法序列数
   > - **非交叉弦图**（圆上画 n 条不交叉弦的方案）
   >
   > 
   >
   > Q:<mark>这个题目考虑了值，是凑巧等于卡特兰数？</mark>
   >
   > ✅ 正确理解
   >
   > 这个问题并不是在问 **有 3 个节点可以构造出多少种不同形状的二叉树**，而是问：
   >
   > > **前序遍历是 A → D → C 的所有可能的二叉树结构有多少种？**
   >
   > 这是个**约束型问题**，比纯粹的结构计数更具体，限制更多，**不等价于卡特兰数 C3=5** —— 但这个题的答案恰好也是 5。
   >
   > 所以：
   >
   > > **这不是“考虑了值”的卡特兰问题，而是“固定前序遍历”的限制型问题，恰好结果与 C3 相同是巧合。**
   >
   > ------
   >
   > 🔢 补充说明：分类对比如下
   ...
   > | 问题                                                  | 节点有值？       | 顺序限制？     | 结果      | 解释                                       |
   > | ----------------------------------------------------- | ---------------- | -------------- | --------- | ------------------------------------------ |
   > | 有 n 个节点，有多少种不同形状的二叉树？               | ❌ 无值           | ❌ 无顺序       | Cn        | 卡特兰数定义                               |
   > | 有 n 个值不同的节点，可以构造多少棵不同的二叉搜索树？ | ✅ 有值           | ✅ 顺序满足 BST | Cn        | BST 结构唯一决定                           |
   > | 有 n 个值不同的节点，构造多少种前序遍历结果？         | ✅ 有值           | ✅ 前序固定     | 小于 Cn   | 结构受限                                   |
   > | 给出具体前序遍历（如 A D C），问可能结构数？          | ✅ 有值且顺序固定 | ✅              | 一般 < Cn | 是约束问题，结果不等于 C_n，但可能碰巧一样 |
   >
   > ------
   >
   > 🎯 总结一句话：
   >
   > > 本题结果为 5 是因为“前序为 A D C”这个特定字符串，构成的所有可能二叉树结构有 5 种，与 C3=5 **数值相同但含义不同**。不能认为这是“考虑了值”的卡特兰数问题。
   >
   > 如你所猜，这是**凑巧**。在别的前序序列下可能不是 5。
